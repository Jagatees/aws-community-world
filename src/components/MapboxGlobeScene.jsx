  ...
  useEffect(() => {
    ...
    const updateOverlay = () => {
      ...
      markersRef.current.forEach(({ cluster, element }) => {
        ...
        element.style.opacity = visible && onScreen ? '1' : '0';
        element.style.visibility = visible && onScreen ? 'visible' : 'hidden';
        element.style.pointerEvents = visible && onScreen ? 'auto' : 'none';
