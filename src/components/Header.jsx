/**
 * @param {{ darkMode: boolean, onToggleDark: () => void, globeStyle: string, onToggleGlobeStyle: () => void }} props
 */
export default function Header({ darkMode, onToggleDark, globeStyle, onToggleGlobeStyle }) {
  const surface = darkMode ? '#1B2836' : '#ffffff';
  const border = darkMode ? '#2D3F50' : '#d0dce8';
  const text = darkMode ? '#FFFFFF' : '#0F1923';
  const muted = darkMode ? '#8B9BAA' : '#5a7a99';
  const isNeon = globeStyle === 'neon';

  return (
    <header
      className="w-full px-4 py-2 flex items-center gap-3"
      style={{ backgroundColor: surface, borderBottom: `1px solid ${border}` }}
    >
      <span style={{ color: '#FF9900' }} className="font-bold text-lg tracking-tight">AWS</span>
      <span style={{ color: border }} className="text-lg select-none">|</span>
      <span style={{ color: text }} className="font-semibold text-base">Community Globe</span>

      <div className="flex-1" />

      <span className="text-xs hidden sm:block" style={{ color: muted }}>
        Created by AWS Cloud Club @ SIT
      </span>

      {/* Globe style toggle */}
      <button
        onClick={onToggleGlobeStyle}
        aria-label={isNeon ? 'Switch to realistic globe' : 'Switch to neon globe'}
        title={isNeon ? 'Realistic mode' : 'Neon mode'}
        className="p-1.5 rounded transition-colors"
        style={{ color: isNeon ? '#FF9900' : muted }}
        onMouseEnter={(e) => (e.currentTarget.style.color = '#FF9900')}
        onMouseLeave={(e) => (e.currentTarget.style.color = isNeon ? '#FF9900' : muted)}
      >
        {/* Globe icon */}
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <circle cx="12" cy="12" r="10" />
          <line x1="2" y1="12" x2="22" y2="12" />
          <path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" />
        </svg>
      </button>

      {/* GitHub link */}
      <a
        href="https://github.com/Jagatees/aws-community-world"
        target="_blank"
        rel="noopener noreferrer"
        aria-label="GitHub repository"
        className="p-1.5 rounded transition-colors"
        style={{ color: muted }}
        onMouseEnter={(e) => (e.currentTarget.style.color = '#FF9900')}
        onMouseLeave={(e) => (e.currentTarget.style.color = muted)}
      >
        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.385-1.335-1.755-1.335-1.755-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 21.795 24 17.295 24 12c0-6.63-5.37-12-12-12z" />
        </svg>
      </a>

      {/* Dark / Light mode toggle */}
      <button
        onClick={onToggleDark}
        aria-label={darkMode ? 'Switch to light mode' : 'Switch to dark mode'}
        className="p-1.5 rounded transition-colors"
        style={{ color: muted }}
        onMouseEnter={(e) => (e.currentTarget.style.color = '#FF9900')}
        onMouseLeave={(e) => (e.currentTarget.style.color = muted)}
      >
        {darkMode ? (
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="12" cy="12" r="5" />
            <line x1="12" y1="1" x2="12" y2="3" /><line x1="12" y1="21" x2="12" y2="23" />
            <line x1="4.22" y1="4.22" x2="5.64" y2="5.64" /><line x1="18.36" y1="18.36" x2="19.78" y2="19.78" />
            <line x1="1" y1="12" x2="3" y2="12" /><line x1="21" y1="12" x2="23" y2="12" />
            <line x1="4.22" y1="19.78" x2="5.64" y2="18.36" /><line x1="18.36" y1="5.64" x2="19.78" y2="4.22" />
          </svg>
        ) : (
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
          </svg>
        )}
      </button>
    </header>
  );
}
  const surface = darkMode ? '#1B2836' : '#ffffff';
  const border = darkMode ? '#2D3F50' : '#d0dce8';
  const text = darkMode ? '#FFFFFF' : '#0F1923';
  const muted = darkMode ? '#8B9BAA' : '#5a7a99';

  return (
    <header
      className="w-full px-4 py-2 flex items-center gap-3"
      style={{ backgroundColor: surface, borderBottom: `1px solid ${border}` }}
    >
      {/* Logo + title */}
      <span style={{ color: '#FF9900' }} className="font-bold text-lg tracking-tight">AWS</span>
      <span style={{ color: border }} className="text-lg select-none">|</span>
      <span style={{ color: text }} className="font-semibold text-base">Community Globe</span>

      <div className="flex-1" />

      {/* Created by */}
      <span className="text-xs hidden sm:block" style={{ color: muted }}>
        Created by AWS Cloud Club @ SIT
      </span>

      {/* GitHub link */}
      <a
        href="https://github.com/Jagatees/aws-community-world"
        target="_blank"
        rel="noopener noreferrer"
        aria-label="GitHub repository"
        className="p-1.5 rounded transition-colors"
        style={{ color: muted }}
        onMouseEnter={(e) => (e.currentTarget.style.color = '#FF9900')}
        onMouseLeave={(e) => (e.currentTarget.style.color = muted)}
      >
        {/* GitHub SVG icon */}
        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.385-1.335-1.755-1.335-1.755-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 21.795 24 17.295 24 12c0-6.63-5.37-12-12-12z" />
        </svg>
      </a>

      {/* Dark / Light mode toggle */}
      <button
        onClick={onToggleDark}
        aria-label={darkMode ? 'Switch to light mode' : 'Switch to dark mode'}
        className="p-1.5 rounded transition-colors"
        style={{ color: muted }}
        onMouseEnter={(e) => (e.currentTarget.style.color = '#FF9900')}
        onMouseLeave={(e) => (e.currentTarget.style.color = muted)}
      >
        {darkMode ? (
          // Sun icon
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="12" cy="12" r="5" />
            <line x1="12" y1="1" x2="12" y2="3" />
            <line x1="12" y1="21" x2="12" y2="23" />
            <line x1="4.22" y1="4.22" x2="5.64" y2="5.64" />
            <line x1="18.36" y1="18.36" x2="19.78" y2="19.78" />
            <line x1="1" y1="12" x2="3" y2="12" />
            <line x1="21" y1="12" x2="23" y2="12" />
            <line x1="4.22" y1="19.78" x2="5.64" y2="18.36" />
            <line x1="18.36" y1="5.64" x2="19.78" y2="4.22" />
          </svg>
        ) : (
          // Moon icon
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
          </svg>
        )}
      </button>
    </header>
  );
}
